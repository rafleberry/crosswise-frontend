...
export const massHarvestFarm = async (masterChefContract) => {
  ...
  try {
    ...
    console.log('massHarvesterror: ', err)
    throw Error(err)
  }
}
...
export const withdrawOutstandingRewards = async (masterChefContract, amount) => {
  const tx = await masterChefContract.withdrawOutstandingCommission(amount)
  const receipt = await tx.wait()
  return receipt.status
}
