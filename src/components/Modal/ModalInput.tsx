    ...
    <div style={{ position: 'relative', marginTop: 5 }}>
      <StyledTokenInput isWarning={isBalanceZero}>
        <Flex justifyContent="space-between" pl="16px">
          ...
          <Text fontSize="14px">{t(`${balanceLabel ?? 'Balance'}: %balance%`, { balance: displayBalance(max) })}</Text>
        </Flex>
        {!!extraString && isBalanceZero && (
          <Text fontSize="14px" textAlign="right">{`Withdrawable ${extraString}`}</Text>
        )}
      ...
      {isBalanceZero && !hideNoneBalanceString && (
