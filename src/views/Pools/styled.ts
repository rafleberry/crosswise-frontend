...
import { Image, Text, Button, Flex, Input } from '@crosswise/uikit'
...
export const FarmHeadCard = styled.div<{ isDarkTheme: boolean; expanded: boolean; isMobile: boolean }>`
  ...
  &:hover {
    ...
    /* border-image: linear-gradient(to right, #3f81ef, #8750f4);
...
export const HeadCardOperationPanelWrapper = styled.div<{ isMobile: boolean }>`
  ...
  max-width: 1400px;
  margin: auto;
  margin-top: ${({ isMobile }) => (isMobile ? '10px' : '30px')};
  position: relative;
  /* opacity: ${({ theme }) => (theme.isDark ? 0.9 : 1)}; */
`

export const FarmHeadCardOperationPanel = styled.div<{ isMobile: boolean }>`
