...
import { ExpandButtonText, ExpandButtonWrapper, StyledSvg } from './styled'
...
const SideWing: React.FC<SideWingProps> = ({ inverse }) => (
  <StyledSvg
    inverse={inverse}
    width="20"
    height="40"
  ...
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M20 20C8.95432 20 0 11.0457 0 0V20C0 31.0457 8.95432 40 20 40V20Z"
      fill="#6F61F3"
    />
  </StyledSvg>
)
...
const ExpandButton: React.FC<Props> = ({ title, onClick, className }) => {
  return (
    <ExpandButtonWrapper className={className}>
      {/* <TopPart /> */}
      <ExpandButtonText onClick={onClick}>{title}</ExpandButtonText>
      <SideWing />
      <SideWing inverse />
    </ExpandButtonWrapper>
  )
