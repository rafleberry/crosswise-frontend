...
export const StyledTr = styled.div<{ index: number; expanded?: boolean; isMobile?: boolean; isLock?: boolean }>`
  cursor: pointer;
  /* opacity: 0;
  ...
  background: ${({ theme }) =>
    theme.isDark
      ...
      : '#FFF'};
  border: ${({ isLock, theme }) =>
    isLock ? '1px solid #EEBF00' : theme.isDark ? '1px solid rgba(224, 224, 255, 0.22)' : 'none'};
