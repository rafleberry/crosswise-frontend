...
import { BaseLayout, Text, Heading, DropDownBottomIcon, CheckmarkIcon, Button } from '@crosswise/uikit'
import { IconCopy } from 'components/SvgIcons'
import { TextBox } from 'components/Texts'

const FONT_FAMILY = 'genos'
...
export const ReferralCardWrapper = styled.div<{ isMobile?: boolean }>`
  ...
  display: grid;
  grid-template-columns: ${({ isMobile }) => (isMobile ? '100%' : '210px calc(100% - 510px) 300px')};
  justify-content: center;
  align-items: center;
...
const expandAnimation = (isMobile) => keyframes`
  from {
    max-height: 0px;
    margin-top: 0px;
  }
  to {
    max-height: 500px;
    margin-top: ${isMobile ? 20 : 39}px;
  }
`

const collapseAnimation = (isMobile) => keyframes`
  from {
    max-height: 500px;
    margin-top: ${isMobile ? 20 : 39}px;
  }
...
export const ReferralDetailInfo = styled.div<{ expanded?: boolean; isMobile?: boolean }>`
  /* animation: ${({ expanded, isMobile }) =>
    expanded ? expandAnimation(isMobile) : collapseAnimation(isMobile)} 300ms
    linear forwards; */
  position: relative;
  display: grid;
...
export const ReferralClaimButton = styled(Button)<{ isMobile: boolean }>`
